## easy-cli

@todo 新版

## update info:

- 2019.02.14: update typeicalbuild.

#### params

- port(p): listen port(default:8443);
- dirname: listen dirname

#### example

```sh
easycli -s -p 3000 --dirname D:/
```

#### proxy.config.js

if folder has 'proxy.config.js', the proxy can add interfaces(Array) from the file.
as

```sh
	// proxy.config.js
	module.exports = {
		target: 'http://www.npm.com',
		urls: [
			'/public/Activity/',
			'/nodetest/'
		]
	}
```

### 2.image build

format image file to webp and blur base64 string.
If fs system find the render HTML file, it will replace `easycli-img="..."` to `src="{{blured base64 image}}" data-format="{{image format}}" data-original="{{image path}}"`

\*need gm and **[ImageMagick](http://www.imagemagick.org/script/download.php)**

```sh
    easycli -m
```

or

```sh
    easycli --image
```

#### params

- config: config file (default: img.config.js).(optional)
- pc: handle pc rendered HTML file.

#### example

```sh
    easycli -m --config src/img.config.js
```

#### \* html image load render

if root directory has 'index.html', imageBuilder will render image label which has 'easycli-img' attribute.

for example

before:

```html
<img easycli-img="images/01.base64" />
```

after:

```html
<img
  src="data:image/png;base64,"
  data-format="png"
  data-original="images/01"
/>
```

- src: base64 blured image;
- data-format: original image format;
- data-original: original image path;

#### get base64 string

example

```sh
    easycli --base64 logo.png
```

if you want to change image type, add type argument

```sh
    easycli --base64 logo.png --type jpg
```

#### get webp image

example

```sh
    easycli --webp logo.png
```

### 3.typical project build

init typical project folder and core files.

```sh
    easycli -t {tasktype}
```

or

```sh
    easycli --typical {tasktype}
```

default folder name: Time string.

#### param

- name: folder name;

#### types

- v/vue: vue project;
- r/react: react project;
- w/webpack: webpack project(no frame, with zepto);
- g+w/gulp+webpack: gulp+webpack project(no frame, with zepto);

#### command

run development:

```sh
	npm run start
```

run build:

```sh
	npm run build
```

run production:

```sh
	npm run build:prod
```

#### example

```sh
    easycli -t webpack --name 2018test
```

## Author

Micheal Wayne

## Update time

2019.02.14
