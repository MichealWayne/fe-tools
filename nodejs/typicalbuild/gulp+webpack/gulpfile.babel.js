...
gulp.task('clean-dev', function () {
  ...
  return gulp.src(Globel.dev, { read: false }).pipe(clean({ force: true }));
  // force: true 即允许 del 控制本目录以外的文件
});
gulp.task('clean-pro', function () {
  console.log('-----清理pro目录完成-----');
  return gulp.src(Globel.pro, { read: false }).pipe(clean({ force: true }));
});
...
gulp.task('rev', function () {
  return gulp
    .src([Globel.pro + '/rev/**/*.json', Globel.pro + '/**/*.html'])
    .pipe(
      revCollector({
        replaceReved: true,
      })
