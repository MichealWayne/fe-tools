...
Builder.buildFiles = data => {
    ...
    FS.setFile(path.join(THIS.dist, THIS.buildname), htmlData, THIS.callback);
};
...
Builder.init = options => {
    ...
    this.callback = config.callback ? () => {
        config.callback(options.folder, _buildname);
    } : () => process.exit();
...
module.exports = {
    init: configData => {
        if (configData) {
            ...
            if (configData.callback) config.callback = configData.callback;
        }
