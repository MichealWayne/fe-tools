...
var HOME_URL = 'https://github.com/MichealWayne/fe-tools';
var DEFAULT_SEARCH_LIST = ['mdn', 'github', 'npmjs', 'caniuse', 'stackoverflow', 'baidu']
var FEToolsData = null; // 请求插件数据
...
var FeTools = {
    setListeners: function () {
        ...
        // focus and blur
        DOMS.input.addEventListener('focus', function () {
            if (DOMS.input.value) DOMS.logoCtn.style.height = '0';
        }, false);
        DOMS.input.addEventListener('blur', function () {
            if (!DOMS.input.value) {
    ...
    fetchListData: function () {
        ...
        // get fe-tools data
        axios.get('https://testfund.10jqka.com.cn/public/fe-tools/data.json').then(function (data) {
    ...
    handleSearch: function (value) {
        ...
        if (~value.indexOf('http')) {
            DOMS.searchList.innerHTML = '<li><a s-cr_blue2 href="' + value + '" target="_blank">前往地址>></a></li>' +
        ...
        var defaultHTML = DEFAULT_SEARCH_LIST.map(function (item) {
            return '<li data-type="' + item + '" data-val="' + value + '"><a>在' + item + '中搜索：<strong>' + value + '</strong></a></li>';
        }).join('');
        DOMS.searchList.innerHTML = resultList.map(function (item) {
            return '<li><a class="u-w" href="' + item.link + '" target="_blank"><em class="u-icon_il icon-label" s-' + item.color + '>' + item.label + '</em><span s-ft_base>' + item.name.replace(value, '<b s-cr_blue>' + value + '</b>') + '</span></a></li>';
        }).join('') || defaultHTML;

        if (resultList.length) { // 其他搜索
    ...
    handleQRCode: function () {
        chrome.tabs.getSelected(null, function (tab) {
            ...
            DOMS.fixCtn.innerHTML = '<div>\
                    <img title="右键可保存二维码图片" class="u-icon" src="' + qrnode.toDataURL("image/png") + '"/>\
                    <button data-url="' + url + '" id="downloadQR" class="u-btn u-w100 u-h40 g-center g-mt20" s-color="blue">下载SVG文件</button>\
                </div>';
    ...
    setEvents: function () {
        // result item click
        DOMS.searchList.addEventListener('click', function (e) {
            var target = e.target,
        ...
        // fix ctn
        DOMS.fixCtn.addEventListener('click', function (e) {
            var target = e.target;
            if (target.id === 'downloadQR') { // download QR code
