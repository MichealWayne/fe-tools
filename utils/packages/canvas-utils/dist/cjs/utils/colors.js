...
exports.getLightfulRgbList = exports.isTransparentColor = exports.getColorRgba = exports.getColorRgbList = void 0;
/**
 ...
 */
function getColorRgbList(color) {
    var _a;
    ...
    // 如果是简写形式，将其转换为完整形式
    var colorFormatted = color.length === 4 ? color.replace(/([0-9a-fA-F])/g, '$1$1') : color;
    // 提取并转换颜色值
    return (((_a = colorFormatted
        .substring(1)
        .match(/.{2}/g)) === null || _a === void 0 ? void 0 : _a.map(function (hex) { return parseInt(hex, 16); })) || []);
}
exports.getColorRgbList = getColorRgbList;
/**
 ...
 */
function getColorRgba(str, opacity) {
    if (opacity === void 0) { opacity = 1; }
    var colorList = getColorRgbList(str);
    var isLegalColor = colorList.length === 3;
    return "rgba(".concat(isLegalColor ? colorList.join(',') : '0,0,0', ",").concat(isLegalColor ? opacity : 0, ")");
}
 ...
 */
function getLightfulRgbList(color, weight) {
    if (weight === void 0) { weight = 0; }
    return getColorRgbList(color).map(function (data) { return Math.min(255, data + Math.round(data * weight)); });
}
exports.getLightfulRgbList = getLightfulRgbList;
