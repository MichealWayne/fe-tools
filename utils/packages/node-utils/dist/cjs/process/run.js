...
exports.commandEvents = exports.exit = exports.runSync = exports.runPromise = exports.forceRunAsync = exports.runAsync = exports.IGNORE = void 0;
/**
...
function runAsync(cmd, args, options) {
    ...
    if (cmd instanceof URL) {
        cmd = (0, url_1.fileURLToPath)(cmd);
    }
    return new Promise(function (resolve, reject) {
        var _a = options.ignoreFailure, ignoreFailure = _a === void 0 ? false : _a, _b = options.captureStdout, captureStdout = _b === void 0 ? false : _b, timeout = options.timeout, spawnOptions = __rest(options, ["ignoreFailure", "captureStdout", "timeout"]);
        var opt = __assign({ cwd: process.cwd(), stdio: captureStdout ? ['inherit', 'pipe', 'pipe'] : 'inherit' }, spawnOptions);
        var child = (0, child_process_1.spawn)(cmd, args, opt);
        var stdoutData = '';
        var stderrData = '';
        var timeoutId = null;
        var settled = false;
        var finalize = function (handler) {
            if (settled)
                return;
            settled = true;
            if (timeoutId)
        ...
        if (timeout) {
            timeoutId = setTimeout(function () {
                child.kill('SIGKILL');
                var err = new Error("Command timed out after ".concat(timeout, "ms: ").concat(cmd, " ").concat(args.join(' ')));
                finalize(function () { return reject(err); });
            }, timeout);
        }
        // 捕获输出
        ...
        child.on('close', function (code) {
            ...
            if (code !== 0 && !ignoreFailure) {
                var err_1 = new Error("Command failed: ".concat(cmd, " ").concat(args.join(' '), "\n").concat(stderrData));
                Object.assign(err_1, result);
                finalize(function () { return reject(err_1); });
            }
            else {
                finalize(function () { return resolve(result); });
            }
        });
        // 处理进程错误
 ...
 */
function forceRunAsync(cmd, args, options) {
    if (args === void 0) { args = []; }
    if (options === void 0) { options = {}; }
    return runAsync(cmd, args, __assign(__assign({}, options), { ignoreFailure: true }));
}
exports.forceRunAsync = forceRunAsync;
/**
 * @function runPromise
...
function runPromise(promise) {
    return promise.catch(function (error) {
        if (error.message !== exports.IGNORE) {
            console.error(error);
        }
        exit();
    });
...
function runSync(cmd, args, options) {
    ...
    var opt = __assign({ cwd: process.cwd(), encoding: 'utf8' }, options);
    try {
        var result = (0, child_process_1.spawnSync)(cmd, args, opt);
        return {
            code: result.status,
            stdout: ((_a = result.stdout) === null || _a === void 0 ? void 0 : _a.toString()) || '',
            stderr: ((_b = result.stderr) === null || _b === void 0 ? void 0 : _b.toString()) || '',
 ...
 */
exports.commandEvents = new events_1.EventEmitter();
exports.default = {
    runAsync: runAsync,
    runSync: runSync,
    forceRunAsync: forceRunAsync,
    runPromise: runPromise,
    exit: exit,
    commandEvents: exports.commandEvents,
};
