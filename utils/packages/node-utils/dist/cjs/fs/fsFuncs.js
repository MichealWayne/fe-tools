...
exports.readJsonFile = exports.readFileSync = exports.writeJson = exports.writeFile = exports.rmdirsSync = exports.setFolderSync = exports.fsExistsSync = exports.mkdirsSync = exports.travelFolderSync = void 0;
 ...
 */
function travelFolderSync(dirPath, fileCallback, folderCallback) {
    if (!fs_1.default.existsSync(dirPath)) {
        tip_1.default.error("Directory not found: ".concat(dirPath));
        return;
    }
    fs_1.default.readdirSync(dirPath).forEach(function (file) {
        var pathName = (0, path_1.join)(dirPath, file);
...
exports.travelFolderSync = travelFolderSync;
/**
 * @function mkdirsSync
...
function mkdirsSync(dirPath) {
    ...
    try {
        ...
        try {
            for (var dirs_1 = __values(dirs), dirs_1_1 = dirs_1.next(); !dirs_1_1.done; dirs_1_1 = dirs_1.next()) {
                var dirName = dirs_1_1.value;
                pathTemp = pathTemp ? (0, path_1.join)(pathTemp, dirName) : dirName;
                if (!fs_1.default.existsSync(pathTemp)) {
                    try {
                        fs_1.default.mkdirSync(pathTemp);
                    }
                    catch (e) {
                        tip_1.default.log("Error! Create directory failed! Path=".concat(pathTemp, " Error: ").concat(e));
                        return false;
                    }
                }
            }
            ...
            try {
                if (dirs_1_1 && !dirs_1_1.done && (_a = dirs_1.return)) _a.call(dirs_1);
            }
        ...
        tip_1.default.log("Error!create director fail! path=".concat(dirPath, " errorMsg:").concat(err));
        return false;
    }
}
...
function setFolderSync(folderPath, noTip) {
    if (noTip === void 0) { noTip = false; }
    if (!fs_1.default.existsSync(folderPath)) {
        mkdirsSync(folderPath);
    }
    else if (!noTip) {
        tip_1.default.log("\r\n(".concat(folderPath, " folder existed.)"));
    }
}
...
function rmdirsSync(targetPath) {
    ...
    try {
        if (fs_1.default.existsSync(targetPath)) {
            var files = fs_1.default.readdirSync(targetPath);
            try {
                for (var files_1 = __values(files), files_1_1 = files_1.next(); !files_1_1.done; files_1_1 = files_1.next()) {
                    var file = files_1_1.value;
                    var curPath = "".concat(targetPath, "/").concat(file);
                    if (fs_1.default.statSync(curPath).isDirectory()) {
                        if (!rmdirsSync(curPath))
                            return false;
                    }
                    else {
            ...
            fs_1.default.rmdirSync(targetPath);
        }
    }
    catch (err) {
        tip_1.default.error("Error! Remove directory failed! Path=".concat(targetPath, ", Error: ").concat(err));
        return false;
    }
    return true;
}
 ...
 * }
 */
function writeFile(filePath, fileData, replace) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!(fsExistsSync(filePath) && !replace)) return [3 /*break*/, 2];
                    return [4 /*yield*/, fs_1.default.promises.appendFile(filePath, fileData)];
                case 1:
                    _a.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, fs_1.default.promises.writeFile(filePath, fileData)];
                case 3:
                    _a.sent();
                    _a.label = 4;
 ...
 */
function writeJson(filePath, obj, spaceLen) {
    if (spaceLen === void 0) { spaceLen = 2; }
    return writeFile(filePath, "".concat(JSON.stringify(obj, null, spaceLen), "\n"));
}
...
function readJsonFile(filePath) {
    try {
        ...
        tip_1.default.error("Failed to parse JSON file: ".concat(filePath, ", error: ").concat(err));
        return {};
    }
    return {};
}
...
exports.default = {
    travelFolderSync: travelFolderSync,
