 ...
 */
export function travelFolderSync(dirPath, fileCallback, folderCallback) {
    if (!fs.existsSync(dirPath)) {
        Tip.error("Directory not found: ".concat(dirPath));
        return;
    }
    fs.readdirSync(dirPath).forEach(function (file) {
        var pathName = join(dirPath, file);
 ...
 */
export function mkdirsSync(dirPath) {
    var e_1, _a;
    try {
        if (fs.existsSync(dirPath)) {
            return true;
            ...
            for (var dirs_1 = __values(dirs), dirs_1_1 = dirs_1.next(); !dirs_1_1.done; dirs_1_1 = dirs_1.next()) {
                ...
                if (!fs.existsSync(pathTemp)) {
                    try {
                        ...
                        Tip.log("Error! Create directory failed! Path=".concat(pathTemp, " Error: ").concat(e));
                        return false;
                    }
                }
            }
        ...
        Tip.log("Error!create director fail! path=".concat(dirPath, " errorMsg:").concat(err));
        return false;
    }
}
/**
 ...
 */
export function fsExistsSync(folderPath) {
    try {
 ...
 */
export function setFolderSync(folderPath, noTip) {
 ...
 */
export function rmdirsSync(targetPath) {
    var e_2, _a;
    try {
        ...
        Tip.error("Error! Remove directory failed! Path=".concat(targetPath, ", Error: ").concat(err));
        return false;
    }
    return true;
}
 ...
 * }
 */
export function writeFile(filePath, fileData, replace) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, fs.promises.appendFile(filePath, fileData)];
                case 1:
                    _a.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, fs.promises.writeFile(filePath, fileData)];
                case 3:
                    _a.sent();
                    _a.label = 4;
 ...
 */
export function writeJson(filePath, obj, spaceLen) {
...
export function readJsonFile(filePath) {
    try {
        ...
        Tip.error("Failed to parse JSON file: ".concat(filePath, ", error: ").concat(err));
        return {};
    }
    return {};
}
