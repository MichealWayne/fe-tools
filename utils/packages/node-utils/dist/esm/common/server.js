...
var startServer = function (serverPath, port, callback) {
    ...
    var folderPath = serverPath || path.join(__dirname, '../');
    var rootPath = path.resolve(folderPath);
    var listenPort = port;
    ...
    var server = http.createServer(function (req, res) {
        ...
        if (filePath !== rootPath && !filePath.startsWith(rootPath + path.sep)) {
            Tip.error("Path traversal blocked: ".concat(safePath));
            res.writeHead(403, { 'content-type': 'text/plain; charset=utf-8' });
            res.end('403 Forbidden');
            return;
        }
        Tip.log("Request: ".concat(reqUrl, " -> ").concat(filePath));
        if (req.method === 'HEAD') {
        ...
        fs.readFile(filePath, function (err, data) {
            if (err) {
                Tip.error("File not found: ".concat(filePath, " (").concat(err, ")"));
    ...
    server.listen(listenPort, function () {
        Tip.safe("Server started! Listening: ".concat(listenPort, " (").concat(url, ")"));
        if (callback)
            callback(url, server);
    });
};
