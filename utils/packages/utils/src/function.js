 ...
 * curry(Math.pow)(2, 10); // -> 1024
 */
export var curry = function (callback) {
    return function () {
        var args = [];
        for (var _i = 0; _i < arguments.length; _i++) {
        ...
        if (args.length < callback.length) {
            return curry(callback.bind.apply(callback, __spreadArray([null], __read(args), false)));
        }
 ...
 * });
 */
export function debounce(fn, intervalTime) {
    if (intervalTime === void 0) { intervalTime = DEFAULT_INTERVAL; }
    var timeId = null;
