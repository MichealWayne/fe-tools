...
export function isIdCard(str: string) {
  // 旧正则 [0|1|2] 在字符类内 | 是字面管道符（非交替），实际匹配 0/|/1/2。
  // 合法身份证不会含 | 所以行为侥幸正确，但写法误导。改为 [012] 更准确。
  // The old [0|1|2] treated | as a literal pipe inside the character class (not alternation),
  // actually matching 0/|/1/2. Valid ID cards never contain | so behavior was correct by luck,
  // but the form was misleading. Use [012] instead.
  return /^(^[1-9]\d{7}((0\d)|(1[0-2]))(([012]\d)|3[0-1])\d{3}$)|(^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([012]\d)|3[0-1])((\d{4})|\d{3}[Xx])$)$/.test(
    str
  );
}
