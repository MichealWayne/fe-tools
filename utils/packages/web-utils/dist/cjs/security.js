"use strict";
/**
 * @author Wayne
 * @Date 2023-05-28 15:45:10
 * @LastEditTime 2025-09-07 21:40:34
 * @description Web 安全工具函数
 */
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __read = (this && this.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
        while ((n === void 0 || n-- > 0) && !(r = i.next()).done) ar.push(r.value);
    }
    catch (error) { e = { error: error }; }
    finally {
        try {
            if (r && !r.done && (m = i["return"])) m.call(i);
        }
        finally { if (e) throw e.error; }
    }
    return ar;
};
var __values = (this && this.__values) || function(o) {
    var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
    if (m) return m.call(o);
    if (o && typeof o.length === "number") return {
        next: function () {
            if (o && i >= o.length) o = void 0;
            return { value: o && o[i++], done: !o };
        }
    };
    throw new TypeError(s ? "Object is not iterable." : "Symbol.iterator is not defined.");
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createBaseCSP = exports.stringifyCSP = exports.parseCSP = exports.addCSRFHeader = exports.getCSRFToken = exports.storeCSRFToken = exports.generateCSRFToken = exports.maskText = exports.maskBankCard = exports.maskIDCard = exports.maskEmail = exports.maskPhoneNumber = exports.generateCSPNonce = exports.sanitizeUrl = exports.unescapeHtml = exports.escapeHtml = void 0;
/**
 * XSS 防御工具
 */
/**
 * @function escapeHtml
 * @description HTML转义函数，防止XSS攻击。HTML escape function to prevent XSS attacks
 * @param {string} str - 需要转义的字符串。The string to escape
 * @returns {string} 转义后的字符串。The escaped string
 */
function escapeHtml(str) {
    if (!str)
        return '';
    return str
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
        .replace(/'/g, '&#39;');
}
exports.escapeHtml = escapeHtml;
/**
 * @function unescapeHtml
 * @description HTML反转义函数。HTML unescape function
 * @param {string} str - 需要反转义的字符串。The string to unescape
 * @returns {string} 反转义后的字符串。The unescaped string
 */
function unescapeHtml(str) {
    if (!str)
        return '';
    return str
        .replace(/&amp;/g, '&')
        .replace(/&lt;/g, '<')
        .replace(/&gt;/g, '>')
        .replace(/&quot;/g, '"')
        .replace(/&#39;/g, "'");
}
exports.unescapeHtml = unescapeHtml;
/**
 * @function sanitizeUrl
 * @description 过滤URL，防止JavaScript伪协议攻击。Filters URLs to prevent JavaScript pseudo-protocol attacks
 * @param {string} url - 需要过滤的URL。The URL to filter
 * @returns {string} 过滤后的URL。The filtered URL
 */
function sanitizeUrl(url) {
    if (!url)
        return '';
    var invalidProtocolRegex = /^([^\w]*)(javascript|data|vbscript|file):/i;
    // 移除控制字符 (ASCII 0-31) 和其他不可见字符
    // 逐个字符检查而不是使用正则表达式
    var sanitizedUrl = Array.from(url)
        .filter(function (char) {
        var code = char.charCodeAt(0);
        // 过滤 ASCII 控制字符 (0-31)、特殊区域 (127-159) 和某些 Unicode 空格字符
        return !(((code >= 0 && code <= 31) ||
            (code >= 127 && code <= 159) ||
            (code >= 8192 && code <= 8205) ||
            code === 65279) // FEFF (Zero Width No-Break Space)
        );
    })
        .join('');
    if (invalidProtocolRegex.test(sanitizedUrl)) {
        return 'about:blank';
    }
    return sanitizedUrl;
}
exports.sanitizeUrl = sanitizeUrl;
/**
 * 创建 CSP nonce 值
 * @returns 随机生成的 nonce 值
 */
function generateCSPNonce() {
    // 生成一个随机的 base64 字符串作为 nonce
    var randomBytes = new Uint8Array(16);
    window.crypto.getRandomValues(randomBytes);
    return btoa(String.fromCharCode.apply(null, Array.from(randomBytes)));
}
exports.generateCSPNonce = generateCSPNonce;
/**
 * 数据脱敏工具
 */
/**
 * 手机号脱敏（保留前 3 位和后 4 位）
 * @param phoneNumber 手机号
 * @returns 脱敏后的手机号
 */
function maskPhoneNumber(phoneNumber) {
    if (!phoneNumber)
        return '';
    var regex = /^(\d{3})(\d*)(\d{4})$/;
    return phoneNumber.replace(regex, '$1****$3');
}
exports.maskPhoneNumber = maskPhoneNumber;
/**
 * 邮箱脱敏（保留用户名首尾字符和域名）
 * @param email 邮箱
 * @returns 脱敏后的邮箱
 */
function maskEmail(email) {
    if (!email)
        return '';
    var _a = __read(email.split('@'), 2), username = _a[0], domain = _a[1];
    if (!domain)
        return email;
    var maskedUsername = username;
    if (username.length > 2) {
        maskedUsername = "".concat(username.charAt(0), "***").concat(username.charAt(username.length - 1));
    }
    return "".concat(maskedUsername, "@").concat(domain);
}
exports.maskEmail = maskEmail;
/**
 * 身份证号脱敏（保留前 4 位和后 4 位）
 * @param idCard 身份证号
 * @returns 脱敏后的身份证号
 */
function maskIDCard(idCard) {
    if (!idCard)
        return '';
    var regex = /^(\d{4})(\d*)(\d{4})$/;
    return idCard.replace(regex, '$1**********$3');
}
exports.maskIDCard = maskIDCard;
/**
 * 银行卡号脱敏（保留后 4 位）
 * @param cardNumber 银行卡号
 * @returns 脱敏后的银行卡号
 */
function maskBankCard(cardNumber) {
    if (!cardNumber)
        return '';
    var regex = /^(\d*)(\d{4})$/;
    return cardNumber.replace(regex, '****$2');
}
exports.maskBankCard = maskBankCard;
/**
 * 通用文本脱敏函数
 * @param text 需要脱敏的文本
 * @param visibleStartChars 开头可见字符数
 * @param visibleEndChars 结尾可见字符数
 * @param mask 遮罩字符，默认为 *
 * @returns 脱敏后的文本
 */
function maskText(text, visibleStartChars, visibleEndChars, mask) {
    if (visibleStartChars === void 0) { visibleStartChars = 0; }
    if (visibleEndChars === void 0) { visibleEndChars = 0; }
    if (mask === void 0) { mask = '*'; }
    if (!text)
        return '';
    var textLength = text.length;
    if (textLength <= visibleStartChars + visibleEndChars) {
        return text;
    }
    var start = text.substring(0, visibleStartChars);
    var end = text.substring(textLength - visibleEndChars);
    var masked = mask.repeat(textLength - visibleStartChars - visibleEndChars);
    return "".concat(start).concat(masked).concat(end);
}
exports.maskText = maskText;
/**
 * CSRF 防护工具
 */
/**
 * 生成 CSRF Token
 * @returns 随机生成的 CSRF Token
 */
function generateCSRFToken() {
    var randomBytes = new Uint8Array(32);
    window.crypto.getRandomValues(randomBytes);
    return Array.from(randomBytes)
        .map(function (byte) { return byte.toString(16).padStart(2, '0'); })
        .join('');
}
exports.generateCSRFToken = generateCSRFToken;
/**
 * 将 CSRF Token 存储到 localStorage
 * @param token CSRF Token
 * @param key 存储的键名，默认为 'csrf_token'
 */
function storeCSRFToken(token, key) {
    if (key === void 0) { key = 'csrf_token'; }
    localStorage.setItem(key, token);
}
exports.storeCSRFToken = storeCSRFToken;
/**
 * 从 localStorage 获取 CSRF Token
 * @param key 存储的键名，默认为 'csrf_token'
 * @returns CSRF Token
 */
function getCSRFToken(key) {
    if (key === void 0) { key = 'csrf_token'; }
    return localStorage.getItem(key) || '';
}
exports.getCSRFToken = getCSRFToken;
/**
 * 请求头中添加 CSRF Token
 * @param headers 请求头对象
 * @param token CSRF Token
 * @returns 添加了 CSRF Token 的请求头对象
 */
function addCSRFHeader(headers, token) {
    var csrfToken = token || getCSRFToken();
    if (csrfToken) {
        return __assign(__assign({}, headers), { 'X-CSRF-Token': csrfToken });
    }
    return headers;
}
exports.addCSRFHeader = addCSRFHeader;
/**
 * 内容安全策略 (CSP) 相关工具
 */
/**
 * 解析 CSP 策略字符串为对象
 * @param cspString CSP 策略字符串
 * @returns 解析后的 CSP 对象
 */
function parseCSP(cspString) {
    var e_1, _a;
    if (!cspString)
        return {};
    var result = {};
    var directives = cspString
        .split(';')
        .map(function (directive) { return directive.trim(); })
        .filter(Boolean);
    try {
        for (var directives_1 = __values(directives), directives_1_1 = directives_1.next(); !directives_1_1.done; directives_1_1 = directives_1.next()) {
            var directive = directives_1_1.value;
            var _b = __read(directive.split(/\s+/)), name_1 = _b[0], values = _b.slice(1);
            if (name_1) {
                result[name_1] = values;
            }
        }
    }
    catch (e_1_1) { e_1 = { error: e_1_1 }; }
    finally {
        try {
            if (directives_1_1 && !directives_1_1.done && (_a = directives_1.return)) _a.call(directives_1);
        }
        finally { if (e_1) throw e_1.error; }
    }
    return result;
}
exports.parseCSP = parseCSP;
/**
 * 将 CSP 对象转换为策略字符串
 * @param cspObject CSP 对象
 * @returns CSP 策略字符串
 */
function stringifyCSP(cspObject) {
    return Object.entries(cspObject)
        .map(function (_a) {
        var _b = __read(_a, 2), name = _b[0], values = _b[1];
        if (values.length === 0) {
            return name;
        }
        return "".concat(name, " ").concat(values.join(' '));
    })
        .join('; ');
}
exports.stringifyCSP = stringifyCSP;
/**
 * 创建基本的 CSP 对象
 * @param nonce 可选的 nonce 值
 * @returns 基本的 CSP 对象
 */
function createBaseCSP(nonce) {
    var csp = {
        'default-src': ["'self'"],
        'script-src': ["'self'"],
        'style-src': ["'self'"],
        'img-src': ["'self'", 'data:'],
        'font-src': ["'self'"],
        'connect-src': ["'self'"],
        'frame-src': ["'none'"],
        'object-src': ["'none'"],
        'base-uri': ["'self'"],
    };
    if (nonce) {
        csp['script-src'].push("'nonce-".concat(nonce, "'"));
        csp['style-src'].push("'nonce-".concat(nonce, "'"));
    }
    return csp;
}
exports.createBaseCSP = createBaseCSP;
