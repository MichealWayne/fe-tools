 ...
 * @returns 脱敏后的文本
 */
export function maskText(text, visibleStartChars, visibleEndChars, mask) {
 ...
 */
export function storeCSRFToken(token, key) {
 ...
 * @returns CSRF Token
 */
export function getCSRFToken(key) {
    if (key === void 0) { key = 'csrf_token'; }
 ...
 */
export function addCSRFHeader(headers, token) {
 ...
 * @returns CSP 策略字符串
 */
export function stringifyCSP(cspObject) {
 ...
 * @returns 基本的 CSP 对象
 */
export function createBaseCSP(nonce) {
    var csp = {
        'default-src': ["'self'"],
        'script-src': ["'self'"],
        'style-src': ["'self'"],
