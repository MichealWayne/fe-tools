...
test('arrayToCSV test', () => {
    expect(arrayToCSV([[1,2,3], [3,4,5]])).toBe(`"1","2","3"
"3","4","5"`);
...
test('compact test', () => {
    ...
    expect(JSON.stringify(compact([1, false, 3, null, 5, undefined, 7, '', 9, NaN, 11]))).toBe('[1,3,5,7,9,11]');
...
test('negate test', () => {
    expect(JSON.stringify([1, 2, 3, 4, 5].filter(negate(n => n % 2 === 0)))).toBe('[1,3,5]');
});
